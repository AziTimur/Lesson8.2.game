...
public class InfinityWar {
    ...
    private static void applySuperAbilityCapAmerica(Boss boss, Hero[] avengers){
        for (int i = 0; i < avengers.length; i++) {
            if (boss.getHealth() > 0 && avengers[i].getHealth() > 0){
                avengers[i].applySuperAbility(boss, avengers);
            }
    ...
    private static void applySuperAbilityIronMan(Boss boss, Hero[] avengers){
        for (int i = 0; i < avengers.length; i++) {
            if (boss.getHealth() > 0 && avengers[i].getHealth() > 0){
                avengers[i].applySuperAbility(boss, avengers);
            }
        }
    }    private static void applySuperAbilityHulk(Boss boss, Hero[] avengers){
        for (int i = 0; i < avengers.length; i++) {
            if (boss.getHealth() > 0 && avengers[i].getHealth() > 0){
                avengers[i].applySuperAbility(boss, avengers);
            }
